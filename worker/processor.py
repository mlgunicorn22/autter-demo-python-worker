...
def build_prompt(user_text: str):
    return f"System: follow company policy. User request: {user_text}"
...
def process_job(job):
    ...
    try:
        if job.kind == "file":
            job.payload["content"] = read_upload(job.payload["path"])
        if job.kind == "llm":
            job.payload["prompt"] = build_prompt(job.payload["text"])
        job.status = "complete"
    except Exception:
        job.status = "failed"
